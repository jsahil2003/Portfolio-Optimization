...
A quantitative long-only equity portfolio built from Nifty 50 + Nifty
Next 50 + Nifty Midcap 100 (200 large-cap and midcap stocks, no
smallcap), backtested 2021-01-01 to 2025-12-31 against a ₹1,00,00,000
starting capital with 0.1% transaction cost per trade, benchmarked
against NIFTY 50 and NIFTY 500 TMI.

> **Note on universe:** the competition brief specifies a 300-stock
> universe (Nifty 100 + Midcap 100 + Smallcap 100). This submission
> deliberately uses the 200-stock universe above instead: a robustness
> check (re-tuning and walk-forward-validating the same approach on the
> full 300-stock universe) found this large-cap + midcap-only universe
> outperforms it on both return and risk-adjusted return, at the cost of
> a deeper max drawdown — see "Universe deviation from brief" in the
> workbook's Model_Logic_Assumptions sheet for the full comparison.

## Strategy

Every month, every eligible stock is scored on four signals, each
z-scored and combined into one composite score. The top 10 stocks by
composite score are held for that month.

| Signal | Weight | What it measures |
|---|---|---|
| Momentum | 55% | 12-month return, most recent month excluded — the dominant signal for this large-cap + midcap universe |
| Low-volatility (close-to-close) | 10% | Trailing 6-month realized volatility, inverted — calmer stocks have historically shown better risk-adjusted returns |
| 52-week-high proximity | 25% | Current price ÷ trailing 12-month high — stocks near their own recent high tend to keep outperforming |
| Liquidity | 10% | Amihud illiquidity, sign-flipped — the strategy trades often enough that easy-to-trade names matter |

Both signals are computed purely from historical price/volume data
...
concentrating in one sector). Both caps are enforced jointly
(`portfolio.apply_caps()`): on the rare month where a 20%-per-stock and
35%-per-sector allocation can't both be satisfied at full investment,
the unallocated remainder is held as cash rather than breaking either
cap. Rebalanced monthly, in whole shares only.

The factor weights (momentum 55% / low-vol 10% / 52-week-high 25% /
liquidity 10%) were chosen via a 1001-combination walk-forward grid
search (step 0.10) over the training window (2021-2023), confirmed on
the unseen test window (2024-2025), then re-centered via a fine-grid
(step 0.05) neighborhood check to avoid landing on an isolated lucky
spike — see [`tune_weights.py`](tune_weights.py) and
[`neighborhood_check.py`](neighborhood_check.py) (results in
`weight_tuning_results.csv` / `neighborhood_check_results.csv`).
...
|---|---|---|---|
| Total Net PNL | ₹7.53 crore | — | — |
| Annualized Return | 55.0% | 13.4% | 15.5% |
| Max Drawdown | -23.2% | -17.2% | -18.8% |
| Sharpe Ratio | 2.03 | 0.97 | 1.07 |
| Gain-to-Loss Ratio | 1.36 | — | — |
| Accuracy (win rate) | 70.6% | — | — |
| Total Trades | 674 | — | — |
| Total Transaction Cost | ₹17.0 lakh | — | — |
...
the strategy isn't overfit to one period) gave Sharpe 2.28 (train) vs.
...
| `universe.py` | Pulls the live 200-stock universe (Nifty 50 + Next 50 + Midcap 100) from niftyindices.com |
| `data.py` | Downloads OHLCV price/volume data and computes returns |
| `factors.py` | Computes the composite selection factors |
...
| `tune_weights.py` | Walk-forward grid search that chose the composite factor weights |
| `neighborhood_check.py` | Fine-grid robustness check around the grid-search winner |
...
To re-run the factor-weight tuning from scratch:

```bash
python tune_weights.py          # 1001-combo walk-forward grid search
python neighborhood_check.py    # fine-grid robustness check around the winner
```
